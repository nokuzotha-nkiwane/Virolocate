...
**nokuzotha-nkiwane/Virolocate** is a Nextflow pipeline for genomic data analysis of Cholera outbreaks.
...
```bash
nextflow run nokuzotha-nkiwane/virolocate -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nokuzotha-nkiwane/virolocate
```
...
First, go to the [nokuzotha-nkiwane/virolocate releases page](https://github.com/nokuzotha-nkiwane/virolocate/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
