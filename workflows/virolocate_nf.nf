...
workflow VIROLOCATE_NF {
   ...
   //sort the filtered list to remove duplicates
    ...
    ch_ncbi_nt_db = Channel.fromPath(params.ncbi_nt_db, checkIfExists: true).map {db -> [[id:"ncbi_nt"], db]}.dump(tag:'ch_ncbi_nt_db').map { it[0] }
    ch_taxids = Channel.value(false).dump(tag:'ch_taxids').map { it[0] }
    ch_taxidlist = Channel.fromPath(params.taxidlist).dump(tag:'ch_taxidlist').map { it[0] }

    ch_negative_tax = Channel.value(false).dump(tag:'ch_negative_tax').map { it[0] }
